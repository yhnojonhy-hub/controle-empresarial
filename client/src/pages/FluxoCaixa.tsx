...
export default function FluxoCaixa() {
  ...
  const { data: fluxos = [], isLoading } =
    trpc.financeiro.fluxoCaixa.list.useQuery();
  ...
  const totalEntradas =
    fluxos
      ...
      .reduce((acc, f) => acc + parseFloat(f.valor || "0"), 0) || 0;
  const totalSaidas =
    fluxos
  ...
  const renderTable = (items: any[]) => (
    <>
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div
          className={`${saldo >= 0 ? "bg-blue-50" : "bg-orange-50"} p-4 rounded-lg`}
        >
            ...
            {items.map(fluxo => (
              ...
              >
                ...
                <td className="py-3 px-4">
                  <Badge
                    variant={
                      fluxo.tipo === "Entrada" ? "default" : "destructive"
                    }
                  >
                    {fluxo.tipo === "Entrada" ? (
                ...
                <td
                  className={`py-3 px-4 text-right font-bold ${fluxo.tipo === "Entrada" ? "text-green-600" : "text-red-600"}`}
                >
                  {fluxo.tipo === "Entrada" ? "+" : "-"} R${" "}
                  {parseFloat(fluxo.valor || "0").toFixed(2)}
                </td>
                  ...
                  <div className="flex justify-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (
                          confirm(
                            `Tem certeza que deseja deletar "${fluxo.descricao}"?`
                          )
                        ) {
