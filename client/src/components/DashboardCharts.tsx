...
export function DashboardCharts({ kpisData, contasData }: DashboardChartsProps) {
  ...
  const faturamentoData = kpisData.map((kpi) => ({
    ...
    lucro: parseFloat(kpi.faturamentoBruto || "0") - parseFloat(kpi.impostos || "0") - parseFloat(kpi.custosFixos || "0") - parseFloat(kpi.custosVariaveis || "0"),
  }));
  ...
  const despesasData = Object.entries(despesasPorCategoria).map(([name, value]) => ({
    name,
    value,
  }));
    ...
    <div className="grid gap-6 md:grid-cols-2">
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={faturamentoData}>
              ...
              <Legend />
              <Line type="monotone" dataKey="faturamento" stroke="#0088FE" name="Faturamento" strokeWidth={2} />
      ...
      <Card>
        ...
        <CardContent>
          <ResponsiveContainer width="100%" height={300}>
            <PieChart>
              <Pie data={despesasData} cx="50%" cy="50%" labelLine={false} label={(entry) => entry.name} outerRadius={80} fill="#8884d8" dataKey="value">
